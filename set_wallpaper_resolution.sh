...
# Installation paths (when script is in /usr/bin/)
...
install_script() {
    if [[ "$EUID" -ne 0 ]]; then
        error_exit "Installation requires root privileges. Please run: sudo $0 --install"
    fi
    
    local current_script="$0"
    
    # Check if we're already in /usr/bin/
    if [[ "$current_script" == "$INSTALL_PATH" ]]; then
        log "INFO" "Script is already installed at $INSTALL_PATH"
        exit 0
    fi
    ...
    if cp "$current_script" "$INSTALL_PATH"; then
        ...
        log "INFO" "Script successfully installed to $INSTALL_PATH"
        log "INFO" "You can now run: $(basename "$INSTALL_PATH") from anywhere"
        exit 0
    else
        error_exit "Failed to install script to $INSTALL_PATH"
    fi
}
usage() {
    cat << EOF
...
Options:
    ...
    --install          Install this script to /usr/bin/ (requires sudo)
...
Examples:
    ...
    sudo $0 --install                    # Install to /usr/bin/ for system-wide access
